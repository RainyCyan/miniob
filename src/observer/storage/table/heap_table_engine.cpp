...
RC HeapTableEngine::create_index(Trx *trx, const vector<FieldMeta> &fields_meta, const char *index_name)
{
  ...
  //add check field
  for(const auto &field:fields_meta)
  {
    if(!field)
    {
      LOG_INFO("Invalid input arguments,field is nullptr");
      return RC::INVALID_ARGUMENT;
    }
  }

  IndexMeta new_index_meta;
  //init index_meta with fields_meta
