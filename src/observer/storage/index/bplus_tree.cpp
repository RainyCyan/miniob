...
RC BplusTreeHandler::create(LogHandler &log_handler, BufferPoolManager &bpm, const char *file_name,
    vector<AttrType> &attr_type, vector<int> &attr_length, int internal_max_size /* = -1*/,
    int leaf_max_size /* = -1 */)
{
...
RC BplusTreeHandler::create(LogHandler &log_handler, DiskBufferPool &buffer_pool, vector<AttrType> &attr_type,
    vector<int> &attr_length, int internal_max_size /* = -1 */, int leaf_max_size /* = -1 */)
{
  ...
  file_header->key_length = accumulate(attr_length.begin(), attr_length.end(), 0) + sizeof(RID);
  // file_header->attr_type         = attr_type;
  file_header->internal_max_size = internal_max_size;
  file_header->leaf_max_size     = leaf_max_size;
  file_header->root_page         = BP_INVALID_PAGE_NUM;

  // init attr_length and attr_type
  ASSERT(attr_type.size()<=MAX_FIELD_NUM,"fields' num greater than max field num");
  file_header->field_num=attr_type.size();
  ...
  //refactor key_comparator and key_printer
