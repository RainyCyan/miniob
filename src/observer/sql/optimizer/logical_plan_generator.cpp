...
RC LogicalPlanGenerator::create_plan(SelectStmt *select_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  // init predicate_oper using filter_expr
  auto &filter_expressions = select_stmt->filter_expressions();
  if (!filter_expressions.empty()) {
    auto predicate_oper_ = unique_ptr<PredicateLogicalOperator>(
        new PredicateLogicalOperator(std::move(select_stmt->filter_expressions()[0])));

    if (predicate_oper_) {
      if (*last_oper) {
        predicate_oper_->add_child(std::move(*last_oper));
      }
      predicate_oper = static_cast<std::unique_ptr<LogicalOperator>>(std::move(predicate_oper_));
...
RC LogicalPlanGenerator::create_plan(DeleteStmt *delete_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  // init predicate_oper using filter_expr
  auto &filter_expressions = delete_stmt->filter_expressions();
  if (!filter_expressions.empty()) {
    predicate_oper = unique_ptr<PredicateLogicalOperator>(
        new PredicateLogicalOperator(std::move(delete_stmt->filter_expressions()[0])));
  }
...
RC LogicalPlanGenerator::create_plan(UpdateStmt *update_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  // 构造谓词算子
  auto                       &filter_expressions = update_stmt->filter_expressions();
  unique_ptr<LogicalOperator> predicate_oper     = nullptr;
  if (!filter_expressions.empty()) {
    predicate_oper = unique_ptr<PredicateLogicalOperator>(
        new PredicateLogicalOperator(std::move(update_stmt->filter_expressions()[0])));
  }
