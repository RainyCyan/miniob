...
UpdateStmt::UpdateStmt(Table *table, Value *values, int value_amount,
    vector<unique_ptr<Expression>> &filter_expressions, std::string attr_name)
    : table_(table),
      values_(values),
      value_amount_(value_amount),
      filter_expressions_(std::move(filter_expressions)),
...
RC UpdateStmt::create(Db *db, UpdateSqlNode &update, Stmt *&stmt)
{
  ...
  RC rc=expression_binder.bind_expression(update.condition, filter_expressions);
  if (OB_FAIL(rc)) {
  ...
  // 目前只支持value为单个的情况
  stmt = new UpdateStmt(table, (Value *)&update.value, 1, filter_expressions, attr_name);
  return RC::SUCCESS;
}
